...
void Arm64FPRCache::FlushRegister(size_t preg, bool maintain_state)
{
  ...
  // We must not spill any registers here, for two reasons:
  // 1. If GetReg needs to spill, it calls this function, potentially causing infinite recursion
  // 2. If maintain_state is true, spilling any register is forbidden
